...
func NewRulerWithJson(data []byte) (*Ruler, error) {
	rules := parseJSONToRules(data)
...
func (r *Ruler) Test(o map[string]interface{}) bool {
	...
	for _, f := range r.rules {
		...
		} else if f.Comparator == "exists" || f.Comparator == "nexists" {
			// either one of these can be done
			return r.compare(f, val)
		} else {
			log.Debugf("did not find property (%s) on map", f.Path)
...
func (r *Ruler) compare(f *Rule, actual interface{}) bool {
	...
	default:
		...
		log.Debugf("unknown comparator %s", f.Comparator)
		return false
	}
}
...
func parseJSONToRules(data []byte) []*Rule {
	if data == nil {
		return nil
	}
	...
	for _, result := range parsed {
		rule := &Rule{
			Comparator: result["comparator"].(string),
			Path:       result["path"].(string),
			Value:      result["value"],
		}
		rules = append(rules, rule)
		// Process value to determine value type
		n, ok := result["value"].(json.Number)
		if !ok {
			continue
		}
